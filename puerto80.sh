...
# Script adicional para configuraciÃ³n de puerto 80
cat > scripts/setup_port80.sh << 'EOF'
#!/bin/bash
# ConfiguraciÃ³n especÃ­fica para puerto 80 en Ubuntu Server

echo "í´§ Configurando aplicaciÃ³n para puerto 80..."

# Verificar permisos de root
if [[ $EUID -ne 0 ]]; then
   echo "âŒ Este script debe ejecutarse como root (sudo)"
   echo "í²¡ Ejecuta: sudo bash scripts/setup_port80.sh"
   exit 1
fi

PROJECT_PATH=$(pwd)
echo "í³ Directorio del proyecto: $PROJECT_PATH"

# 1. Actualizar rutas en configuraciones
echo "âš™ï¸ Actualizando rutas en configuraciones..."
...
nginx -t
if [[ $? -ne 0 ]]; then
    echo "âŒ Error en configuraciÃ³n de Nginx"
    exit 1
fi

# 3. Configurar servicio systemd
echo "í´§ Configurando servicio Django..."
...
echo "í»¡ï¸ Configurando firewall..."
...
if systemctl is-active --quiet nginx; then
    ...
    echo "âŒ Nginx: ERROR"
    tail -10 /var/log/nginx/error.log
fi

# 8. Probar puerto 80
echo "í¼ Probando puerto 80..."
sleep 2
if curl -s --max-time 5 -I http://localhost | grep -q "HTTP"; then
    echo "í¾‰ Â¡Ã‰XITO! AplicaciÃ³n funcionando en puerto 80"
    echo ""
    echo "í¼ Accede a tu aplicaciÃ³n en:"
    echo "   - http://localhost"
    echo "   - http://$(hostname -I | awk '{print $1}')"
else
    echo "âŒ Error: No se puede acceder por puerto 80"
    echo ""
    echo "í´ Para debugging:"
...
- **Probar conexiÃ³n**: `curl -I http://localhost`

## ResoluciÃ³n de Problemas Puerto 80
...
sudo fuser -k 80/tcp  # Liberar puerto si estÃ¡ ocupado
```
